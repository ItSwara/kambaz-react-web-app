...
export default function Dashboard({
  courses = [], // Default to empty array to prevent "map is not a function" error
  newCourse,
  editMode,
  setNewCourse,
  setEditMode,
  ...
  // For debugging
  console.log("Courses:", courses, "Type:", Array.isArray(courses) ? "Array" : typeof courses);
  
  // Convert courses to array if it's an object and filter out nulls
  const coursesArray: Course[] = Array.isArray(courses) 
    ? courses.filter(course => course !== null) 
    : (typeof courses === 'object' && courses !== null)
      ? Object.values(courses as Record<string, Course>).filter(course => course !== null)
      : [];
            ...
            coursesArray.map((course) => (
              <div key={course._id} className="col">
                <div className="card h-100">
                  <Link
                    to={userRole === "FACULTY" || course.enrolled || !enrolling
                      ? `/Kambaz/Courses/${course._id}/Home`
                      : "#"}
                    className="text-decoration-none text-dark"
                    onClick={(e) => {
                      if (userRole !== "FACULTY" && !course.enrolled && enrolling) {
                        e.preventDefault();
...
//   // Convert courses to array if it's an object
//   const coursesArray: Course[] = Array.isArray(courses) 
//     ? courses 
//     : (typeof courses === 'object' && courses !== null)
//       ? Object.values(courses as Record<string, Course>)
//       : [];
