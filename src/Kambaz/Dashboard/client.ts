...
export const fetchUserEnrollments = async (userId: string): Promise<Enrollment[]> => {
  const response = await axiosWithCredentials.get(`/api/users/${userId}/enrollments`);
  return response.data;
};
...
// In your client.ts file, update these functions:

import { enrollCourse, unenrollCourse } from './actions'; // Import your Redux actions
// import store from '../store'; // Import your Redux store

// API endpoints for enrollments
export const enrollIntoCourse = async (userId: string, courseId: string): Promise<Enrollment> => {
  try {
    const response = await axiosWithCredentials.post(`/api/courses/${courseId}/enroll`);
    
    // Dispatch Redux action to update global state
    store.dispatch(enrollCourse(userId, courseId));
...
export const unenrollFromCourse = async (userId: string, courseId: string): Promise<void> => {
  try {
    await axiosWithCredentials.delete(`/api/courses/${courseId}/enroll`);
    
    // Dispatch Redux action to update global state
    store.dispatch(unenrollCourse(userId, courseId));
  } catch (error) {
